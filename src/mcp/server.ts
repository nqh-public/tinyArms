...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case 'review_code': {
        const result = await reviewCode(args as Record<string, unknown>);
        return {
          content: [
            {
              type: 'text' as const,
      ...
      case 'organize_files': {
        const result = await organizeFiles(args as Record<string, unknown>);
        return {
          content: [
            {
              type: 'text' as const,
      ...
      case 'research_context': {
        const result = await researchContext(args as Record<string, unknown>);
        return {
          content: [
            {
              type: 'text' as const,
